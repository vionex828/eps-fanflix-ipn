...
const defaults = {
  msg3:     'FanFlix: আপনার {product} subscription 3 din por shesh hobe. Renew korun: fanflixbd.com',
  msg1:     'FanFlix: আপনার {product} subscription kal shesh hobe! Renew korun: fanflixbd.com',
  followup: 'FanFlix: Apnar order pending ache! Payment korun: {link}',
};
  ...
  try {
    ...
    if (pendingOrder.followup_sent === 1) {
      await sendTelegram(
        ...
        `_Order removed from unpaid list_ ✅`
      );
    }
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(msg.chat.id,
    `👋 *FanFlix Bot v4.0*\n\n📋 *Commands:*\n` +
    `/customers /expiring /today /revenue /stats\n` +
    `/product /retention /top /pending\n` +
    `/search 01874... /add /edit /delete /export\n\n` +
    `⚙️ *SMS:* /setmsg3 /setmsg1 /setfollowup`,
    { parse_mode: 'Markdown' });
});
...
bot.onText(/\/setfollowup/, (msg) => {
  if (!isOwner(msg)) return;
  bot.sendMessage(msg.chat.id, `Current:\n_${getSetting('followup')}_\n\nNew message ({link}):`, { parse_mode: 'Markdown' });
  ...
  try {
    ...
    if (unpaid.length) {
      ...
      for (const o of unpaid) {
        try {
          await sendSMS(o.phone, formatSMS(getSetting('followup'), { link: config.EPS_PAYMENT_LINK }));
          db.prepare('UPDATE pending_orders SET followup_sent=1 WHERE id=?').run(o.id);
        } catch(e) { console.error('Followup SMS:', e.message); }
      }
      await sendTelegram(`✅ Follow-up SMS sent to ${unpaid.length} unpaid orders.`);
    }
